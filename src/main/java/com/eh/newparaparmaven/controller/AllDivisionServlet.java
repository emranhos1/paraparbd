...
public class AllDivisionServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            conrs = GetDivision.getDivisionTable();
            ...
            rs.last();
            orgRow = rs.getRow();
            divisionId = new int[orgRow];
            divisionName = new String[orgRow];
            createdDate = new String[orgRow];
            rs.beforeFirst();
            int i = 0;
            while (rs.next()) {
                divisionId[i] = rs.getInt("division_id");
            ...
            Logger.getLogger(AllDivisionServlet.class.getName()).log(Level.SEVERE, null, ex);
        } finally {
            try {
                pstm.close();
                rs.close();
