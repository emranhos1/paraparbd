...
public class AllInactiveRentACarServlet extends HttpServlet {

    private conRs getInactiveRentACar;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            getInactiveRentACar = GetRentACarTable.getInactiveRentACar();
            
            con = getInactiveRentACar.getCon();
            rs = getInactiveRentACar.getRs();
            pstm = getInactiveRentACar.getPstm();
           ...
           for (i = 0; i < orgRow; i++) {
                ...
                response.getWriter().write("<tr>"
                        ...
                        + "<a data-toggle='modal' data-alluserid='" + allUserId[i] + "' class='btn btn-primary open-spceDialog-active' href='#addSpecActive' >Active</a>"
                        + "</button>        "
                        + "</td>"
                        + "</tr>");
            }
        } catch (SQLException ex) {
            Logger.getLogger(AllInactiveRentACarServlet.class.getName()).log(Level.SEVERE, null, ex);
        } finally{
            try {
                con.close();
                rs.close();
